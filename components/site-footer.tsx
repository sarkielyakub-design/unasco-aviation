import Link from "next/link";
import { Mail, MapPin, Phone } from "lucide-react";
import { Logo } from "@/components/logo";
import { navLinks, siteConfig } from "@/lib/site";

const services = [
  { label: "Flight Support Services", href: "/aviation-services" },
  { label: "Air Cargo Transportation", href: "/cargo-services" },
  { label: "Airline Management", href: "/aviation-services" },
  { label: "Cargo & Logistics", href: "/logistics" },
  { label: "Travel & Tours", href: "/travel" },
];

export function SiteFooter() {
  return (
    <footer className="bg-[#2F3337] text-white">
      <div className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid gap-12 lg:grid-cols-4">
          {/* =====================================================
              COMPANY
          ====================================================== */}

          <div>
            <Logo inverted />

            <p className="mt-6 text-sm leading-7 text-white/70">
              UNASCO Aviation Limited is a Nigerian aviation company
              providing Flight Support Services, Flight Operations,
              Airline Management, Air Cargo Transportation, Logistics
              Solutions, and General Aviation with professionalism,
              safety and operational excellence.
            </p>

            {/* SOCIAL LINKS */}

            <div className="mt-8 flex flex-wrap gap-3">
              {["LinkedIn", "Facebook", "Instagram", "X"].map((item) => (
                <a
                  key={item}
                  href="#"
                  className="rounded-lg border border-white/20 px-4 py-2 text-sm text-white/75 transition-all duration-300 hover:border-[#C9828D] hover:bg-[#561923] hover:text-white"
                >
                  {item}
                </a>
              ))}
            </div>
          </div>

          {/* =====================================================
              COMPANY LINKS
          ====================================================== */}

          <div>
            <h3 className="text-lg font-semibold text-white">
              Company
            </h3>

            <ul className="mt-6 space-y-4">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-white/65 transition-colors duration-300 hover:text-[#C9828D]"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* =====================================================
              SERVICES
          ====================================================== */}

          <div>
            <h3 className="text-lg font-semibold text-white">
              Our Services
            </h3>

            <ul className="mt-6 space-y-4">
              {services.map((service) => (
                <li key={service.label}>
                  <Link
                    href={service.href}
                    className="text-white/65 transition-colors duration-300 hover:text-[#C9828D]"
                  >
                    {service.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* =====================================================
              CONTACT INFORMATION
          ====================================================== */}

          <div>
            <h3 className="text-lg font-semibold text-white">
              Contact Information
            </h3>

            <div className="mt-6 space-y-6">
              {/* HEAD OFFICE */}

              <div className="flex items-start gap-3">
                <MapPin className="mt-1 h-5 w-5 shrink-0 text-[#C9828D]" />

                <div>
                  <h4 className="font-semibold text-white">
                    Head Office
                  </h4>

                  <p className="mt-1 text-sm leading-6 text-white/65">
                    No. 7 Bompai Road,
                    <br />
                    Kano State,
                    <br />
                    Nigeria.
                  </p>
                </div>
              </div>

              {/* CEO */}

              <div className="flex items-start gap-3">
                <Phone className="mt-1 h-5 w-5 shrink-0 text-[#C9828D]" />

                <div>
                  <h4 className="font-semibold text-white">
                    Chief Executive Officer
                  </h4>

                  <a
                    href="tel:+966556011122"
                    className="mt-1 block text-white/70 transition-colors duration-300 hover:text-[#C9828D]"
                  >
                    +966 556 011 122
                  </a>

                  <p className="text-sm text-white/45">
                    Executive &amp; Corporate Enquiries
                  </p>
                </div>
              </div>

              {/* SALES MANAGER */}

              <div className="flex items-start gap-3">
                <Phone className="mt-1 h-5 w-5 shrink-0 text-[#C9828D]" />

                <div>
                  <h4 className="font-semibold text-white">
                    Director Sales &amp; Marketing
                  </h4>

                  <a
                    href="tel:+2348063332227"
                    className="mt-1 block text-white/70 transition-colors duration-300 hover:text-[#C9828D]"
                  >
                    +234 806 333 2227
                  </a>

                  <p className="text-sm text-white/45">
                    Hafiz Umar Ballah
                  </p>
                </div>
              </div>

              {/* OFFICIAL EMAIL */}

              <div className="flex items-start gap-3">
                <Mail className="mt-1 h-5 w-5 shrink-0 text-[#C9828D]" />

                <div>
                  <h4 className="font-semibold text-white">
                    Official Email
                  </h4>

                  <a
                    href="mailto:unascoaviationltd@gmail.com"
                    className="mt-1 block break-all text-white/70 transition-colors duration-300 hover:text-[#C9828D]"
                  >
                    unascoaviationltd@gmail.com
                  </a>

                  <p className="text-sm text-white/45">
                    Corporate &amp; General Enquiries
                  </p>
                </div>
              </div>

              {/* CUSTOMER SUPPORT */}

              <div className="flex items-start gap-3">
                <Mail className="mt-1 h-5 w-5 shrink-0 text-[#C9828D]" />

                <div>
                  <h4 className="font-semibold text-white">
                    Customer Support
                  </h4>

                  <a
                    href="mailto:unascol2025@gmail.com"
                    className="mt-1 block break-all text-white/70 transition-colors duration-300 hover:text-[#C9828D]"
                  >
                    unascol2025@gmail.com
                  </a>

                  <p className="text-sm text-white/45">
                    Customer Support &amp; Service Enquiries
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* =====================================================
            BOTTOM FOOTER
        ====================================================== */}

        <div className="mt-14 border-t border-white/10 pt-8">
          <div className="flex flex-col items-center justify-between gap-5 text-sm md:flex-row">
            <p className="text-white/50">
              © {new Date().getFullYear()} {siteConfig.name}. All
              Rights Reserved.
            </p>

            <div className="flex gap-6">
              <Link
                href="/privacy-policy"
                className="text-white/50 transition-colors duration-300 hover:text-[#C9828D]"
              >
                Privacy Policy
              </Link>

              <Link
                href="/terms"
                className="text-white/50 transition-colors duration-300 hover:text-[#C9828D]"
              >
                Terms &amp; Conditions
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}